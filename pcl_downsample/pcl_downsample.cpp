...
extern "C"{
    void pcl_downSample(double *array_points, double *array_color, size_t n_points, float leaf_size, double *downsample_points, double *downsample_colors){
        ...
        for (int i = 0; i < n_points; i++) {
            ...
            point.r = array_color[i * cols + 0] * std::numeric_limits<uint8_t>::max();;  // r color
            point.g = array_color[i * cols + 1] * std::numeric_limits<uint8_t>::max();;  // g color
            point.b = array_color[i * cols + 2] * std::numeric_limits<uint8_t>::max();;  // b color
        ...
        for (int i = 0; i < downsampled_cloud->points.size(); i++) {
            const pcl::PointXYZRGB& point = downsampled_cloud->points[i];
            downsample_points[i * 3 + 0] = point.x;
            downsample_points[i * 3 + 1] = point.y;
            downsample_points[i * 3 + 2] = point.z;
            downsample_colors[i * 3 + 0] = point.r;
            downsample_colors[i * 3 + 1] = point.g;
            downsample_colors[i * 3 + 2] = point.b;
        }
